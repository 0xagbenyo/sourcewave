...
import { buildErpDocPdfApiUrl } from '../utils/erpDocPdfUrl';
...
/** Full-screen authenticated PDF preview with download/share. */
export const ErpDocumentPdfModal: React.FC<Props> = ({
  visible,
  doctype,
  docName,
  ...
  const pdfUri = name && dt ? buildErpDocPdfApiUrl(dt, name, printFormat) : '';
  const title = name ? `${name}.pdf` : 'Document';
