...
export const CustomerPaymentsScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = salesInvoiceName
        ? await client.listPaymentEntriesForSalesInvoice(salesInvoiceName, opts)
  ...
  useEffect(() => {
    ...
    void load();
  }, [load, cidLoading, salesInvoiceName]);
  ...
  const openRow = (item: any) => {
    (navigation as { navigate: (name: string, params: object) => void }).navigate('PaymentEntryDetail', {
      name: item.name,
    });
  };

  const renderItem = ({ item }: { item: any }) => {
    const kind = paymentEntryPayKind(item);
    const chip = chipColors(kind);
    const accent = accentForPayKind(kind);
    const statusLabel = paymentEntryStatusLabel(item, kind);
      ...
      >
        ...
        <View style={styles.rowBody}>
          <View style={styles.rowTop}>
            ...
            <Text style={styles.rowAmount}>{money(undefined, item.received_amount ?? item.paid_amount)}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  focusStripLabel: { fontSize: 11, fontWeight: '700', color: '#F57F17', textTransform: 'uppercase' },
  focusStripName: { fontSize: 14, fontWeight: '700', color: Colors.BLACK, marginTop: 2 },
  focusStripClear: { fontSize: 13, fontWeight: '700', color: Colors.WINE },
  filterCard: {
    marginHorizontal: Spacing.MD,
    marginTop: Spacing.MD,
    marginBottom: 8,
